...
export default function PluginManager({ onClose }) {
  ...
  const externals = plugins.filter((p) => p.author !== 'built-in');

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-pm-panel border border-pm-border rounded-xl shadow-2xl w-[600px] max-h-[82vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-pm-border flex-shrink-0">
          <div className="flex items-center gap-5">
            <TabBtn active={tab === 'installed'} onClick={() => setTab('installed')}>
              Installed{' '}
              <span className="ml-1 text-gray-600 font-normal">({plugins.length})</span>
            </TabBtn>
            <TabBtn active={tab === 'store'} onClick={() => setTab('store')}>
              Store
              <span className="ml-1.5 text-xs align-middle bg-pm-accent/20 text-pm-accent border border-pm-accent/30 rounded px-1 py-px font-normal">
                GitHub
              </span>
            </TabBtn>
          </div>
...
function TabBtn({ active, onClick, children }) {
  return (
    <button
      onClick={onClick}
      className={`text-sm font-semibold transition-fast ${
...
function InstalledTab({ builtins, externals, enabledPlugins, setPluginEnabled }) {
  return (
    <div className="space-y-5">
      ...
      <section>
        ...
        {externals.length === 0 ? (
          <div className="text-xs text-gray-600 px-1">
            ...
            tab to download plugins from GitHub.
          </div>
        ) : (
          <div className="space-y-2">
      ...
      <section className="bg-pm-hover/30 border border-pm-border/50 rounded-lg p-4">
        <div className="text-xs font-semibold text-gray-300 mb-1.5">Adding Plugins Manually</div>
        ...
        <div className="text-xs text-gray-600 font-mono bg-pm-bg rounded p-2 leading-relaxed">
          ...
          {'      '}├── renderer.js<br />
          {'      '}└── main.js&nbsp;&nbsp;&nbsp;
          <span className="text-gray-700">(optional IPC handlers)</span>
        </div>
  ...
  return (
    <div
      className={`flex items-center gap-3 p-3 rounded-lg border transition-fast ${
        enabled ? 'border-pm-border bg-pm-bg/50' : 'border-pm-border/30 bg-pm-bg/20 opacity-50'
      }`}
    >
      ...
      <label className="flex-shrink-0 relative cursor-pointer">
        ...
        />
        <div className={`w-9 h-5 rounded-full transition-colors duration-200 ${enabled ? 'bg-pm-accent' : 'bg-gray-700'}`} />
